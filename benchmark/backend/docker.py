...
class DockerBackend(Backend):
    ...
    def initialize(self, engine: Engine, container: Text) -> Container:
        conf = engine.get_config("client")
        logger.info("Initializing %s %s: %s", engine, container, conf)
        # TODO: use DockerContainer only, as we do not distinguish server/client anymore
        client = DockerClient(conf, self)
        self.containers.append(client)
        return client

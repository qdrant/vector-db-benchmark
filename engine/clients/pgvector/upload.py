...
from engine.clients.pgvector.config import PGVECTOR_TABLE_NAME, DISTANCE_INDEX_MAPPING, get_pgvector_connection_string
...
class PGVectorUploader(BaseUploader):
    ...
    def upload_batch(
        ...
        upload_batch_string = "INSERT INTO {} (id, embedding) VALUES %s".format(PGVECTOR_TABLE_NAME)
    ...
    def post_upload(cls, distance):
        ...
        with cls.client.cursor() as cur:
            ...
            print(f"""CREATE INDEX ON {PGVECTOR_TABLE_NAME} 
                        USING ivfflat (embedding {index_type}) 
                        WITH (lists={num_lists});""")
            cur.execute(f"""CREATE INDEX ON {PGVECTOR_TABLE_NAME} 
                        USING ivfflat (embedding {index_type}) 
                        WITH (lists={num_lists});""")
